...
class EnhancedModuleProcessor:
    ...
    def __init__(self):
        ...
        # Inicializar CPL Devastador Protocol
        try:
            from .cpl_devastador_protocol import CPLDevastadorProtocol
            self.cpl_protocol = CPLDevastadorProtocol()
            logger.info("✅ CPL Devastador Protocol inicializado")
        except ImportError as e:
            logger.warning(f"⚠️ CPL Devastador Protocol não disponível: {e}")
            self.cpl_protocol = None
...
"""
    ...
    def generate_all_modules(self, session_id: str, topic: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 1. Carrega dados da Etapa 1 - BUSCA PELOS ARQUIVOS REAIS
            import glob
            
            # Busca por arquivos de consolidação da Etapa 1
            consolidacao_files = []
            search_patterns = [
                f"analyses_data/workflow/{session_id}/consolidacao_etapa1_final_*.json",
                f"analyses_data/workflow/{session_id}/etapa1_concluida_*.json",
                f"analyses_data/{session_id}/consolidacao_etapa1_final_*.json",
                f"analyses_data/{session_id}/etapa1_concluida_*.json"
            ]
            
            for pattern in search_patterns:
                consolidacao_files.extend(glob.glob(pattern))
            
            # Carrega dados consolidados da Etapa 1
            etapa1_data = {}
            markdown_content = "# Dados da Etapa 1\n\n"
            
            if consolidacao_files:
                # Ordena por data de modificação (mais recente primeiro)
                consolidacao_files.sort(key=os.path.getmtime, reverse=True)
                consolidacao_file = consolidacao_files[0]
                
                logger.info(f"📁 Carregando dados da Etapa 1 de: {consolidacao_file}")
                
                with open(consolidacao_file, 'r', encoding='utf-8') as f:
                    etapa1_data = json.load(f)
                
                # Converte dados para markdown para compatibilidade
                markdown_content += f"## Consolidação da Etapa 1\n\n"
                markdown_content += f"**Session ID:** {etapa1_data.get('session_id', session_id)}\n\n"
                
                # Inclui dados virais
                viral_results_files = etapa1_data.get('viral_results_files', [])
                if viral_results_files:
                    markdown_content += f"### Dados Virais ({len(viral_results_files)} arquivos)\n\n"
                    for viral_file in viral_results_files:
                        viral_data = viral_file.get('dados', {})
                        markdown_content += f"- **{viral_file.get('arquivo', 'N/A')}**: {viral_data.get('total_images_found', 0)} imagens encontradas\n"
                
                # Inclui dados de busca massiva
                res_busca_files = etapa1_data.get('res_busca_files', [])
                if res_busca_files:
                    markdown_content += f"\n### Dados de Busca Massiva ({len(res_busca_files)} arquivos)\n\n"
                    for res_file in res_busca_files:
                        res_data = res_file.get('dados', {})
                        markdown_content += f"- **{res_file.get('arquivo', 'N/A')}**: {res_data.get('metadata', {}).get('size_kb', 0):.1f}KB\n"
                
                # Inclui trechos extraídos
                trechos_extraidos = etapa1_data.get('trechos_extraidos', [])
                if trechos_extraidos:
                    markdown_content += f"\n### Trechos Extraídos ({len(trechos_extraidos)} arquivos)\n\n"
                    for trecho_file in trechos_extraidos:
                        trecho_data = trecho_file.get('dados', {})
                        markdown_content += f"- **{trecho_file.get('arquivo', 'N/A')}**: {len(trecho_data.get('trechos', []))} trechos\n"
                
                logger.info(f"✅ Dados da Etapa 1 carregados: {len(markdown_content)} chars")
            else:
                logger.warning(f"⚠️ Nenhum arquivo de consolidação da Etapa 1 encontrado para sessão {session_id}")
                markdown_content += "**AVISO:** Dados da Etapa 1 não encontrados. Usando dados de fallback.\n\n"
            
            # 2. Carrega dados da Etapa 2 - BUSCA PELOS ARQUIVOS REAIS
            etapa2_files = []
            etapa2_patterns = [
                f"analyses_data/workflow/{session_id}/etapa2_concluida_*.json",
                f"analyses_data/{session_id}/etapa2_concluida_*.json",
                f"analyses_data/workflow/{session_id}/synthesis_*.json",
                f"analyses_data/{session_id}/synthesis_*.json"
            ]
            
            for pattern in etapa2_patterns:
                etapa2_files.extend(glob.glob(pattern))
            
            synthesis_data = {}
            
            if etapa2_files:
                # Ordena por data de modificação (mais recente primeiro)
                etapa2_files.sort(key=os.path.getmtime, reverse=True)
                etapa2_file = etapa2_files[0]
                
                logger.info(f"📁 Carregando dados da Etapa 2 de: {etapa2_file}")
                
                with open(etapa2_file, 'r', encoding='utf-8') as f:
                    synthesis_data = json.load(f)
                
                logger.info(f"✅ Dados da Etapa 2 carregados: {len(str(synthesis_data))} chars")
            else:
                logger.warning(f"⚠️ Nenhum arquivo da Etapa 2 encontrado para sessão {session_id}")
                synthesis_data = {
                    "synthesis": "Dados da Etapa 2 não encontrados",
                    "session_id": session_id,
                    "warning": "Etapa 2 não foi executada ou dados não foram salvos corretamente"
                }
            
            logger.info(f"📚 Dados carregados - MD: {len(markdown_content)} chars, JSON: {len(str(synthesis_data))} chars")
            
            # 3. Prepara contexto consolidado COM DADOS REAIS
            context = {
                "topic": topic,
                "session_id": session_id,
                "markdown_content": markdown_content,
                "synthesis_data": synthesis_data,
                "etapa1_data": etapa1_data,  # Dados completos da Etapa 1
                "timestamp": datetime.now().isoformat(),
                # Dados específicos para processamento
                "viral_results_files": etapa1_data.get('viral_results_files', []),
                "res_busca_files": etapa1_data.get('res_busca_files', []),
                "trechos_extraidos": etapa1_data.get('trechos_extraidos', []),
                "dados_viral_integration": etapa1_data.get('dados_viral_integration', {}),
                "estatisticas": etapa1_data.get('estatisticas', {})
            }
            ...
            for module_name, method in module_methods:
                try:
                    ...
                    # Cria massive_data REAL baseado nos dados carregados das etapas anteriores
                    real_massive_data = {
                        "web_search_data": {"consolidated": markdown_content},
                        "statistics": etapa1_data.get('estatisticas', {"total_sources": 0}),
                        ...
                        "dados_viral_integration": etapa1_data.get('dados_viral_integration', {}),
                        "social_media_data": {},
                        # Extrai dados dos arquivos RES_BUSCA
                        "busca_massiva": {
                    ...
                    # Popula dados de busca massiva dos arquivos RES_BUSCA
                    for res_file in etapa1_data.get('res_busca_files', []):
                        res_data = res_file.get('dados', {})
                        if 'busca_massiva' in res_data:
                            busca_data = res_data['busca_massiva']
                            real_massive_data['busca_massiva']['alibaba_websailor_results'].extend(
                                busca_data.get('alibaba_websailor_results', [])
                            )
                            real_massive_data['busca_massiva']['real_search_orchestrator_results'].extend(
                                busca_data.get('real_search_orchestrator_results', [])
                            )
                    
                    # Popula dados sociais dos arquivos virais
                    for viral_file in etapa1_data.get('viral_results_files', []):
                        viral_data = viral_file.get('dados', {})
                        if 'viral_images' in viral_data:
                            real_massive_data['social_media_data']['viral_images'] = viral_data['viral_images']
                    
                    module_result = method(real_massive_data, context, session_id)
